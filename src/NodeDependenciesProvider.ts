...
export class NodeDependenciesProvider implements vscode.WebviewViewProvider {
    ...
    private async refreshDependencies() {
        ...
        try {
            ...
            this._view.webview.postMessage({
                type: "error",
                value: "Failed to parse package.json",
            });
        }
    }
    ...
    private getPackageInfo(
        ...
        return new Promise((resolve) => {
            // Get dist-tags specificially for proper classification of 'latest'
            cp.exec(
                    ...
                    if (err) {
                        ...
                        try {
                            ...
                            if (Array.isArray(data)) {
                                // npm view returns array if getting multiple fields sometimes? no, usually object
                                // But if user requested versions dist-tags --json it returns:
                                // { versions: [...], "dist-tags": { latest: ... } }
                                // Let's handle just in case it's weird
                                versions = data;
                            } else {
                            ...
                            // Fallback logic if dist-tags failed
                            if (latest === "Unknown" && versions.length > 0) {
                                // Find highest version that doesn't have a hyphen (prerelease)
                            ...
                            // Return last 100 versions reversed
    ...
    private searchDependencies(query: string) {
        // eslint-disable-next-line curly
        if (!query) return;
            ...
            (err, stdout) => {
                if (err) {
                    ...
                    try {
                        ...
                        const simpleResults = results.map((r: any) => ({
                            name: r.name,
                            version: r.version,
